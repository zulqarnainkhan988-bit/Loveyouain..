...
try{
  ...
  setTimeout(()=>{ alert('Firebase init failed: ' + (e && e.message ? e.message : e)); }, 300);
}
...
async function cGet(key){
  if(!sharedDocRef){ alert('Not connected to database'); return null; }
  try{
    const snap = await withTimeout(sharedDocRef.get());
    const data = snap.exists ? snap.data() : {};
    return data[key] ?? null;
  }catch(e){ alert('Load failed: ' + (e && e.message ? e.message : e)); return null; }
}
async function cSet(key, value){
  if(!sharedDocRef){ alert('Not connected to database'); return false; }
  try{ await withTimeout(sharedDocRef.set({ [key]: value }, { merge: true })); return true; }
  catch(e){ alert('Save failed: ' + (e && e.message ? e.message : e)); return false; }
}
...
async function saveThoughts(list){ return await cSet('thoughts', list); }
...
function initThoughts(){
  ...
  document.getElementById('thoughtSend').onclick = async ()=>{
    try{
      ...
      if(!selectedWho){ alert('Pick who you are first — tap عین or نین above the box.'); return; }
      if(!msg){ alert('Type something first.'); return; }
      const all = await getThoughts();
      all.push({id:uid(), who:selectedWho, msg, ts:Date.now(), reacts:{}, comments:[]});
      const ok = await saveThoughts(all);
      if(!ok){ return; } // cSet already alerted with the specific error
